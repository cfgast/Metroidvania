...
| `VulkanRenderer.h/.cpp` | Vulkan 1.3 stub implementation. All pure-virtual `Renderer` methods are implemented as no-ops (draw calls do nothing, `loadTexture`/`loadFont` return 0). Creates a GLFW window with `GLFW_NO_API` (no OpenGL context) ready for Vulkan surface creation. Contains a private `VulkanInput` stub implementing `InputSystem` — processes GLFW window events (`glfwPollEvents`) so the window is responsive, but key/gamepad queries return false/0. Selected at compile time via `#define USE_VULKAN` in `main.cpp`. |
...
All rendering and input go through the `Renderer` and `InputSystem` abstractions. The default back-end is `GLRenderer` + `GLFWInput`. A stub `VulkanRenderer` is also compiled and can be activated by defining `USE_VULKAN` at compile time; it currently shows a blank window with no rendering. All math types use GLM and custom types from `src/Math/Types.h`.
