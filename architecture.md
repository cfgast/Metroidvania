...
- **Player**: `InputComponent` → `PhysicsComponent` → `RenderComponent` → `AnimationComponent` → `HealthComponent` → `CombatComponent` → `LightComponent`. Death callback teleports to spawn and refills HP. LightComponent provides the player's dynamic light (warm white point light, registered before draw calls). AnimationComponent tint color is set on setup and on each level-up via `getArmorTint()` (Level 1: white, 2: bronze, 3: silver, 4: gold, 5: platinum).
