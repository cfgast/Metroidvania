...
All rendering and input go through the `Renderer` and `InputSystem` abstractions. The default back-end is `GLRenderer` + `GLFWInput`. `VulkanRenderer` is also compiled and can be activated by defining `USE_VULKAN` at compile time; it has a working frame loop that clears to a solid color each frame using Vulkan 1.3 dynamic rendering. The flat-color graphics pipeline is created and ready for use by future draw-call implementations. Vertex buffer infrastructure is in place: a GPU-local unit quad buffer for flat-color rects and per-frame dynamic buffers for variable geometry (sprites, text, circles, etc.). Draw-call methods (drawRect, drawSprite, etc.) are still no-ops pending pipeline binding and draw commands. All math types use GLM and custom types from `src/Math/Types.h`.
