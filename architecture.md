...
├── assets/                    # Sprite sheets (player base + 4 armor tiers, slime), shaders (GLSL→SPIR-V)
...
│   ├── generate_armor_sprites.py  # Generates armored player sprite sheets (tiers 2-5)
│   └── run-tasks.ps1          # Automation: runs ImplementationPlan tasks via Copilot
└── third_party/               # PhysX and stb_image vendored libraries
```
...
| `AnimationComponent` | Frame-based sprite animation from sprite sheets. Uses `Renderer::TextureHandle` for lazy-loaded textures. Frame rects stored as plain `{x,y,w,h}` structs. Supports per-instance `setTintColor(r,g,b)`. On level-up, animations are re-registered with the armor-tier sprite sheet via `updatePlayerArmorSprites()` in `main.cpp`, which swaps the atlas path so the lazy-loader picks up the new texture. | `Renderer` |
| `HealthComponent` | HP pool with `takeDamage` / `heal`. Renders green→red HP bar via `Renderer::drawRect()` / `drawRectOutlined()`. Fires `onDeath` callback on lethal damage and `onDamage` callback on every hit. | `Renderer` |
| `CombatComponent` | Player melee: AABB hitbox in front of player, animated arc VFX via `Renderer::drawTriangleStrip()` / `drawLines()`, per-swing hit tracking. Supports a **charged spin-slash** (unlocked at level 3+): hold the attack button to charge (pulsing glow via `drawCircle`); release after 0.6 s for a spin slash (1.5× reach, 2× damage, wider arc, 0.4 s duration, dedicated "spin-slash-right/left" animation); release early for a normal attack. Charge is cancelled if the player takes damage (`onDamageTaken()` called from HealthComponent's `onDamage` callback). | `InputComponent`, `PhysicsComponent`, `HealthComponent` (enemies), `AnimationComponent`, `Renderer` |
...
`DebugMenu` — F1-toggled overlay with "Open Map…" button that launches a Windows native file dialog to hot-load any map JSON. Also displays Vulkan diagnostics: GPU device name, GPU frame time (ms, from timestamp queries), validation error count (highlighted red when >0), and validation warning count (highlighted yellow when >0). Renders via `Renderer&`; `handleEvent()` takes `const InputEvent&`.
...
```
 ...
 │  PhysXWorld::init()                         │
 │  InputBindings::load()                      │
 │  Create VulkanRenderer (800×600, 60 FPS)    │
...
|---|---|---|
| GLFW 3.4 | FetchContent (Git) | Windowing, input, Vulkan surface |
| Vulkan SDK 1.3 | System (`find_package(Vulkan)`) | Rendering API |
| VMA 3.1.0 | FetchContent (Git) | Vulkan memory allocation |
| vk-bootstrap 1.3.295 | FetchContent (Git) | Vulkan instance/device/swap chain setup |
| stb_image | Vendored in `third_party/stb/` | Image loading for textures |
| FreeType 2.13.3 | FetchContent (Git) | Font rasterization for text rendering |
...
Post-build steps copy PhysX DLLs, `maps/`, and `assets/` next to the executable. If Python 3 is found, a CMake custom command runs `tools/generate_armor_sprites.py` to regenerate `assets/player_armor_{2..5}.png` whenever the base `player_spritesheet.png` or the script itself changes.
