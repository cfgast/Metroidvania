...
| `VulkanRenderer.h/.cpp` | Vulkan 1.3 implementation (in progress). Initializes a full Vulkan instance (with validation layers in debug builds), selects a discrete GPU via vk-bootstrap, creates a logical device with graphics and present queues, and builds a swap chain (`VK_PRESENT_MODE_FIFO`, `VK_FORMAT_B8G8R8A8_SRGB` preferred). Swap chain images and image views are stored for use by future rendering tasks. Supports swap chain recreation (e.g. on resize). All draw-call methods remain no-ops pending later tasks. Creates a GLFW window with `GLFW_NO_API`. Contains a private `VulkanInput` stub implementing `InputSystem`. Selected at compile time via `#define USE_VULKAN` in `main.cpp`. |
...
All rendering and input go through the `Renderer` and `InputSystem` abstractions. The default back-end is `GLRenderer` + `GLFWInput`. `VulkanRenderer` is also compiled and can be activated by defining `USE_VULKAN` at compile time; it initializes a Vulkan instance, device, and swap chain but draw calls are still no-ops pending future tasks. All math types use GLM and custom types from `src/Math/Types.h`.
